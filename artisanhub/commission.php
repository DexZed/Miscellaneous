...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nameErr) && empty($emailErr) && empty($descriptionErr)) {
        ...
        $sql_check = "SELECT * FROM commission WHERE name = '$name' AND email = '$email' AND description = '$description'";
        $result_check = $conn->query($sql_check);

        if ($result_check->num_rows == 0) {
            // SQL to insert form data into the commission table
            $sql = "INSERT INTO commission (name, email, description) VALUES ('$name', '$email', '$description')";

            // Execute the SQL query
            if ($conn->query($sql) === TRUE) {
                '<script>alert("Commission submitted successfully");
            ...
            echo '<script>alert("Commission already exists");
                window.location.href = "commission.php";</script>';
        }
    }
